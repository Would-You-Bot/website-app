    ...
    <div className="flex flex-wrap gap-8 justify-center">
      <TooltipProvider delayDuration={0}>
        {data.map((cluster, index) => {
          const isAnyShardDown = cluster.some(shard => shard.status !== Status.Ready);

          const areAllShardsDown = cluster.every(shard => shard.status === Status.Disconnected);
            ...
            <div className="flex gap-3 items-center text-xl font-bold text-foregorund">
              ...
              <span
                className={cn(
                  ...
                  isAnyShardDown ? 'bg-status-yellow'
                  : areAllShardsDown ? 'bg-status-red'
                  : 'bg-status-green'
                )}
              ></span>
            </div>
