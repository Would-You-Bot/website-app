  ...
  return (
    <Dialog>
      <DialogTrigger asChild>
        <Button
          className={cn(
            ...
            {
              'bg-gradient-to-r from-purple-500 to-pink-500': featured
            }
          )}
        >
          <ExternalLink className="mr-2 h-4 w-4" /> <span>Use pack</span>
        </Button>
      </DialogTrigger>
      <DialogContent className="md:min-w-max">
        ...
        <section className="grid gap-3 grid-cols-2 sm:grid-cols-3 lg:grid-cols-3 my-1">
          ...
          <div className="flex flex-col gap-1">
            <h3>Type</h3>
            <p className="text-muted-foreground capitalize text-sm">{type}</p>
          </div>
        </section>
        ...
        <section>
          ...
          <div className="border rounded-xl overflow-hidden">
            <div className="dark:bg-[#1D1D1D] bg-background-light flex justify-between px-4 py-2 gap-4 border-b">
              <div className="flex items-center gap-4">
                <p className="block w-fit text-sm">Question</p>
                <div className="relative">
                  <Search className="size-4 absolute left-2 bottom-3 dark:text-[#666666]" />
                  <Input
                    placeholder="Search questions"
                    className="lg:w-[441px] pl-8 focus:ring-0"
                  />
                </div>
              </div>
            </div>
